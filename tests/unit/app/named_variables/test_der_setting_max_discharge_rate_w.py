...
@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_discharge_rate_w_empty(pg_empty_config):
    ...
    async with begin_session() as session:
        with pytest.raises(UnresolvableVariableError, match="DERSetting"):
            await resolvers.resolve_named_variable_der_setting_max_discharge_rate_w(session)


@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_discharge_rate_w_no_setting(pg_base_config):
...
@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_discharge_rate_w_single_setting(pg_base_config):
    ...
    async with begin_session() as session:
        result = await resolvers.resolve_named_variable_der_setting_max_discharge_rate_w(session)
        assert isinstance(result, float)
        assert result == 123.45


@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_discharge_rate_w_many_settings(pg_base_config):
