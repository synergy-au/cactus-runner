...
@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_wh_empty(pg_empty_config):
    ...
    async with begin_session() as session:
        with pytest.raises(UnresolvableVariableError, match="DERSetting"):
            await resolvers.resolve_named_variable_der_setting_max_wh(session)


@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_wh_no_setting(pg_base_config):
    ...
    async with begin_session() as session:
        with pytest.raises(UnresolvableVariableError, match="setMaxWh"):
            await resolvers.resolve_named_variable_der_setting_max_wh(session)


@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_wh_single_setting(pg_base_config):
                ...
                site_ders=[
                    generate_class_instance(
                        ...
                        site_der_setting=generate_class_instance(
                            ...
                            site_der_id=None,
                            max_wh_value=max_wh_value,
                            max_wh_multiplier=max_wh_multiplier,
                        ),
                    )
                ],
            )
    ...
    async with begin_session() as session:
        result = await resolvers.resolve_named_variable_der_setting_max_wh(session)
        assert isinstance(result, float)
        assert result == 123.45


@pytest.mark.asyncio
async def test_resolve_named_variable_der_setting_max_wh_many_settings(pg_base_config):
