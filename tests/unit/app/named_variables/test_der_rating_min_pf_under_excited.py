...
@pytest.mark.asyncio
async def test_resolve_named_variable_der_rating_min_pf_under_excited(pg_empty_config):
    ...
    async with begin_session() as session:
        with pytest.raises(UnresolvableVariableError, match="DERCapability"):
            await resolvers.resolve_named_variable_der_rating_min_pf_under_excited(session)
...
@pytest.mark.asyncio
async def test_resolve_named_variable_der_rating_min_pf_under_excited_single_rating(pg_base_config):
    """If there is a single DERCapability in the db  - return it"""
    min_pf_under_excited_displacement = 950
    min_pf_under_excited_multiplier = -3
    async with begin_session() as session:
        session.add(
            generate_class_instance(
                Site,
    ...
    async with begin_session() as session:
        result = await resolvers.resolve_named_variable_der_rating_min_pf_under_excited(session)
        assert isinstance(result, float)
        assert result == 0.95


@pytest.mark.asyncio
async def test_resolve_named_variable_der_rating_min_pf_under_excited_many_ratings(pg_base_config):
    """If there are multiple DERCapabilities - return the most recent DERCapability"""
    min_pf_under_excited_displacement = 45
    min_pf_under_excited_multiplier = -2
    async with begin_session() as session:
        session.add(
            generate_class_instance(
                Site,
