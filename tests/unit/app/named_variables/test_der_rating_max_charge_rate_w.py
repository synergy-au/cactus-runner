...
async def test_resolve_named_variable_der_rating_max_charge_rate_w_empty(pg_empty_config):
    ...
    async with begin_session() as session:
        with pytest.raises(UnresolvableVariableError, match="DERCapability"):
            await resolvers.resolve_named_variable_der_rating_max_charge_rate_w(session)
...
async def test_resolve_named_variable_der_rating_max_charge_rate_w_single_setting(pg_base_config):
    ...
    async with begin_session() as session:
        result = await resolvers.resolve_named_variable_der_rating_max_charge_rate_w(session)
