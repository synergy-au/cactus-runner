...
@pytest.mark.asyncio
async def test_resolve_named_variable_der_rating_max_wh_empty(pg_empty_config):
    ...
    async with begin_session() as session:
        with pytest.raises(UnresolvableVariableError, match="DERCapability"):
            await resolvers.resolve_named_variable_der_rating_max_wh(session)


@pytest.mark.asyncio
async def test_resolve_named_variable_der_rating_max_wh_no_setting(pg_base_config):
    ...
    async with begin_session() as session:
        with pytest.raises(UnresolvableVariableError, match="rtgMaxWh"):
            await resolvers.resolve_named_variable_der_rating_max_wh(session)


@pytest.mark.asyncio
async def test_resolve_named_variable_der_rating_max_wh_single_setting(pg_base_config):
    ...
    async with begin_session() as session:
        result = await resolvers.resolve_named_variable_der_rating_max_wh(session)
        assert isinstance(result, float)
        assert result == 123.45


@pytest.mark.asyncio
async def test_resolve_named_variable_der_rating_max_wh_many_settings(pg_base_config):
