...
async def test_all_01_with_readings(
    ...
    mup_post_data = mup_agg_xml if certificate_type == "aggregator_certificate" else mup_xml
    result = await cactus_runner_client.post(
        "/mup", data=mup_post_data, headers={"ssl-client-cert": URI_ENCODED_CERT, "Content-Type": env.HEADER_MEDIA_ALL}
    )
